...
class TestUInt32(unittest.TestCase):
  ...
  def test_evaluate_maximum_value_big_endianness(self):
    ...
    valid, value = tested.evaluate(bites=[0xFF, 0xFF, 0xFF, 0xFE], endianness="big")
    self.assertTrue(valid)
    self.assertEqual(4294967294, value)

  def test_evaluate_maximum_value_little_endianness(self):
    tested = UInt32()

    valid, value = tested.evaluate(bites=[0xFE, 0xFF, 0xFF, 0xFF], endianness="little")
    self.assertTrue(valid)
    self.assertEqual(4294967294, value)
